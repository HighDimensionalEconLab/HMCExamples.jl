...
for ((SEED=$START_SEED; SEED<=$NUM_SEEDS+$START_SEED-1; SEED++)); do
  for T in "${T_VALUES[@]}"; do
    ...
    run_sampler $SEED $NUM_SAMPLES "fit_rbc_2_joint.jl" "${RESULTS_PATH}/rbc_2_joint_frequentist_seed_${SEED}_${T}" "${DATA_PATH}/rbc_frequentist/rbc_1_seed_${SEED}_${T}.csv" "${INIT_PARAMS_PATH}/rbc_2_joint_init_params_${T}.csv" 
  done
done
