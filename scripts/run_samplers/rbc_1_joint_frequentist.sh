...
for ((SEED=$START_SEED; SEED<=$NUM_SEEDS+$START_SEED-1; SEED++)); do
  for T in "${T_VALUES[@]}"; do
    ...
    run_sampler $SEED $NUM_SAMPLES "fit_rbc_1_joint.jl" "${RESULTS_PATH}/rbc_1_joint_frequentist_${SEED}_${T}" "${DATA_PATH}/rbc_frequentist/rbc_1_seed_${SEED}_${T}.csv" "${DATA_PATH}/rbc_1_joint_burnin_ergodic_${T}.csv" 
  done
done
