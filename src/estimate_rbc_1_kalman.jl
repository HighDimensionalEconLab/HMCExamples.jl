...
function main_rbc_1_kalman(args=ARGS)
    ...
    return estimate_rbc_1_kalman((; d...)) # to named tuple
end
...
function estimate_rbc_1_kalman(d)
    ...
    chain = (d.num_chains == 1) ? sample(turing_model, sampler,
        d.num_samples; init_params, d.progress, save_state=true, discard_initial = d.discard_initial) : sample(turing_model, sampler, MCMCThreads(), d.num_samples, d.num_chains; init_params=[init_params for _ in 1:d.num_chains], d.progress, save_state=true)
